...
class Estimate_coordinate(Node):
    def __init__(self):
        super().__init__('estimate_coordinate_node_fix')
        ...
        self.target = PoseArray().poses# x =[547,888] y=[245,264]
        self.target.append(Pose())
        self.target.append(Pose())
        self.target[0].position.x = 547.0
        self.target[0].position.y = 245.0
        self.target[1].position.x = 767.0
        self.target[1].position.y = 239.0
    ...
    def timer_callback(self):
        ...
        if self.start[0] and self.start[1] and self.start[2]:
            ...
            for i in range(len(self.target)):
                ...
                cv.putText(Image, 'x: ' + str(self.point[0]), (int(self.pos.x)+20, int(self.pos.y) - 40), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                cv.putText(Image, 'y: ' + str(self.point[1]), (int(self.pos.x)+20, int(self.pos.y) ), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                cv.putText(Image, 'z: ' + str(self.point[2]), (int(self.pos.x)+20, int(self.pos.y) +40), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            # cv.imshow('Image', Image)
            # cv.imshow('depth', depth_image*10)
            cv.imshow('depth', cv.resize(depth_image, (640, 480))*10)
            cv.imshow('Image', cv.resize(Image, (640, 480)))
            cv.waitKey(1)
